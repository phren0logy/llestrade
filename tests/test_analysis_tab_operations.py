#!/usr/bin/env python
"""
Test suite for AnalysisTab UI operations and workflow integration.

This module tests:
1. File operations (summarization, combining, integration)
2. UI workflow and state management
3. Directory and file handling 
4. Error handling and user feedback

Does NOT test:
- Worker thread internals (see test_llm_summarization_thread.py, test_integrated_analysis_thread.py)
...
mock_qtimer_class_obj = MagicMock()
...
class TestAnalysisTabFileOperations(unittest.TestCase):
    """Test core file operations and workflow in AnalysisTab."""
    ...
    def setUp(self, MockLLMSummaryThreadFromDecorator, mock_open_func, mock_makedirs, mock_listdir, mock_path_exists):
        ...
        # Mock the initialization to prevent actual UI creation
        with patch.object(AnalysisTab, '__init__', return_value=None):
            self.tab = AnalysisTab.__new__(AnalysisTab)
        ...
        # Mock new async scanning attributes
        self.tab.directory_scanner_thread = None
        self.tab.scan_progress_dialog = None
        self.tab.cached_markdown_files = {}
        
        # Pre-populate cache for consistent test behavior
        self.tab.cached_markdown_files["/fake/markdown_dir"] = [
            "/fake/markdown_dir/annual_report.md",
            "/fake/markdown_dir/project_alpha.v2.doc.md"
        ]
    ...
    def test_summarize_with_llm_writes_to_summaries_subdir(self):
        # Since we're using cached files now, this should work with the pre-populated cache
        ...
        expected_thread_calls = [
            call(
                self.tab, 
                markdown_files=["/fake/markdown_dir/annual_report.md"], 
            ...
            call(
                self.tab, 
                markdown_files=["/fake/markdown_dir/project_alpha.v2.doc.md"], 
                output_dir=expected_summaries_output_dir,
                subject_name="John Doe",
                subject_dob="2000-01-01",
                case_info="Test case.",
    ...
    def test_generate_integrated_analysis_reads_combined_from_summaries_subdir(self, MockIntegratedAnalysisThread):
        ...
        MockIntegratedAnalysisThread.assert_called_once_with(
            ...
            ["/fake/markdown_dir/annual_report.md", "/fake/markdown_dir/project_alpha.v2.doc.md"],
    ...
    # ==========================================
    # Tests for File List Management
    # ==========================================
    ...
    def test_on_markdown_folder_selected_with_cache(self):
        """Test folder selection when directory is already cached."""
        # Create a mock AnalysisTab instance directly
        tab = MagicMock()
        directory = '/fake/dir'
        tab.cached_markdown_files = {directory: ['file1.md']}
        tab.show_status = MagicMock()
        tab.check_workflow_state = MagicMock()
        tab.status_panel = MagicMock()
        tab.markdown_directory = None
        
        # Bind the method to the mock object
        tab.on_markdown_folder_selected = AnalysisTab.on_markdown_folder_selected.__get__(tab, AnalysisTab)
        
        tab.on_markdown_folder_selected(directory)
        
        self.assertEqual(tab.markdown_directory, directory)
        tab.show_status.assert_called_once()
        tab.check_workflow_state.assert_called_once()

    def test_on_markdown_folder_selected_starts_scan(self):
        """Test folder selection when directory needs to be scanned.""" 
        # Create a mock AnalysisTab instance directly
        tab = MagicMock()
        tab.cached_markdown_files = {}
        tab.start_directory_scan = MagicMock()
        tab.status_panel = MagicMock()
        tab.markdown_directory = None
        ...
        self.assertEqual(tab.markdown_directory, directory)
        tab.start_directory_scan.assert_called_once_with(directory)
