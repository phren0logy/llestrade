...
from llm_utils import combine_transcript_with_fragments, LLMClient
...
class PromptsTab(BaseTab):
    ...
    def on_markdown_selected(self, file_path):
        ...
        self.check_ready_state()
        self.workflow_indicator.update_status(0, [], "Markdown template selected")
    ...
    def load_markdown_preview(self):
        ...
        # Display preview
        self.results_viewer.set_content(f"Preview of {os.path.basename(self.current_markdown_path)}:\n\n{content}")
    ...
    def load_transcript_preview(self):
        ...
        # Display preview
        self.results_viewer.set_content(f"Preview of {os.path.basename(self.current_transcript_path)}:\n\n{content}")
    ...
    def process_template(self):
        ...
        if not self.current_markdown_path or not self.current_transcript_path:
            self.show_status("Please select both a markdown template and transcript file")
            return
        
        try:
            # Show processing status
            self.show_status(f"Processing template: {os.path.basename(self.current_markdown_path)}")
            self.status_panel.append_details(f"Reading markdown file: {self.current_markdown_path}")
            ...
            for template in self.current_templates:
                ...
                prompt = {
                    ...
                    "content": combine_transcript_with_fragments(transcript_content, template["content"])
                }
    ...
    def run_prompts_and_save(self):
        ...
        try:
            ...
            progress_dialog = ProgressDialog(
                "Running Prompts", 
                "Processing prompts through Claude...",
                parent=self
            )
            ...
            # Connect signals
            self.prompt_runner_thread.progress_signal.connect(progress_dialog.update_progress)
            self.prompt_runner_thread.finished_signal.connect(self.on_prompt_runner_finished)
